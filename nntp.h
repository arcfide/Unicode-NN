...
#define OK_AUTH		281	/* Authentication completed OK */

#define CONT_XFER	335	/* Continue to send article */
#define	CONT_POST	340	/* Continue to post article */
#define OK_NEEDPASS	381	/* Authentication username ok, need passwd */
...
#define ERR_NEEDAUTH	480	/* need to authenticate with "authinfo" */
...
void            nntp_check(void);
int             nntp_set_group(group_header *);
FILE           *nntp_get_newsgroups(void);
FILE           *nntp_get_article(article_number, int);
char           *nntp_get_filename(article_number, group_header *);
void            nntp_close_server(void);
void            nntp_cleanup(void);
int             nntp_post(char *temp_file);

#ifdef NOV
struct novgroup *nntp_get_overview(group_header *, article_number, article_number);
FILE           *nntp_fopen_list(char *);
char           *nntp_fgets(char *, int);
#else
int             nntp_get_active(void);
article_number *nntp_get_article_list(group_header *);
#endif
